...
int xmp_rmdir(const char *path)
{
	...
	// Removing from client cache as well...
	...
	//Also removing it from the tmp folder
	string tmpPath = tmpDir + string(path);
		     ...
		     off_t offset, struct fuse_file_info *fi)
{
	...
	//Client Fail 1 condition - the file should be just written to temp folder and NOT to client and server persistent storage.
	if (fail_flag == 1) {
	     kill(getpid(), SIGKILL);
	}
...
int main(int argc, char *argv[])
{
	...
	//Considering -d option used for logging
	if (argc == 5) {
	     num = 2; 
	...
	tmpDir = "/users/ygovind/fuse-2.9.4/example/cs739_p2/tmp/";
	clientCacheDirectory = argv[argc - num];

	cout << "clientCacheDir = " << clientCacheDirectory << endl;
	//clientTmpDirectory = "/tmp";
	fuse_stat = fuse_main(argc - num, args, &xmp_oper, NULL);
